...
# 数据写频率 --step 为300秒(即5分钟为一个数据点)
rrd=rrdtool.create('Flow.rrd','--step','300','--start',cur_time,

    # 定义数据源eth0_in入流量,eth0_out出流量,类型都为COUNTER(递增),600秒为心跳值.
    # 其含义是600秒没有收到值,则会用unknown代替，0为最小值,最大值用U代替,表示为不确定
    'DS:eth0_in:COUNTER:600:0:U',
    'DS:eth0_out:COUNTER:600:0:U',

    # RRA定义格式为[RRA:CF:xff:steps:rows]
    # CF定义了AVERAGE,MAX,MIN三种数据合并方式
    # xff定义为0.5，表示一个CDP中的PDP值如果超过一半值为UNKNOWN，则该CDP的值就被标为UNKNOWN
